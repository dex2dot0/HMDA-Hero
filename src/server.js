...
async function runLocalServer() {
	try {
		if (!process.env.KEYPATH || !process.env.CERTPATH) {
			console.error(
				'You must install certs prior to running a local build. Run "npm run certs" to install locally. See the README for more info.'
			);
		...
		createServer(options, handler).listen(ssl_port, (_) => {
			console.log(
				`> Running on https://localhost:${ssl_port}. You must explicitly open to https://localhost:443. Server should not listen on port 3000, ignore error`
			);
		});
	} catch (error) {}
}
