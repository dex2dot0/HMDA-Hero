<script>
	...
	async function formatActionDates(actionTakenDateArray) {
		...
		for (let i = 0; i < actionTakenDateArray.length; i++) {
			...
			try {
				formattedDates.push(`${date.slice(4, 6)}/${date.slice(6)}/${date.slice(0, 4)}`);
			} catch (error) {
				console.log(error);
				formattedDates.push('');
			}
	...
	async function editLoan(ULI) {
		...
		try {
			...
			if (error.message.includes('Excel is not defined')) {
				console.log(`Excel doesn't exist outside the add-in, returning boilerplate data`);
			} else {
				console.log(error);
			}
		}
			...
			<form class="form-inline">
				...
				{#if selected === 'loanType'}
					...
					<select
						...
						<option value="FHA"> 2. Federal Housing Adminitration insured (FHA) </option>
				...
				{:else if selected === 'actionTaken'}
					...
					<select
						...
						<option value="ANA"> 2. Application approved but not accepted </option>
						<option value="Denied">3. Application denied</option>
						<option value="Withdrawn">4. App withdrawn by applicant</option>
						<option value="Incomplete"> 5. File closed for incompleteness </option>
						<option value="Purchased">6. Purchased loan</option>
						<option value="Pre-App Denied"> 7. Preapproval request denied </option>
			...
			{#if $LEI === '' && selected == 'loanNumber'}
				<form class="shadow-sm mb-5 bg-white rounded">
					<h5 style="color:#dc3545; background-color:#eee;">
						You cannot search by Loan Number until you have entered your LEI in the
						settings menu
					</h5>
				</form>
			{:else}
				<form class="shadow-sm p-1 mb-5 bg-white rounded">
					Click the <strong>Edit Loan</strong> button on a result below to open the loan
					for editing
