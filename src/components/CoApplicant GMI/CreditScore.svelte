<script>
  import { coapplicantCreditScore, NoCoApp } from './../../stores.js';
...
<DataFieldFormat>
  ...
  <span slot="helpers">
    ...
    <Checkbox
      text="Not a #"
      value="{$coapplicantCreditScore == '7777' ? true : false}"
      on:notify="{coapplicantCreditScore.NAN}" />
    <Checkbox
      value="{$coapplicantCreditScore == '8888' ? true : false}"
      on:notify="{coapplicantCreditScore.NA}" />
    <Checkbox
      text="Exempt"
